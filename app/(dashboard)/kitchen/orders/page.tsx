...
export default function KitchenOrdersPage() {
  ...
  const loadOrders = (status: 'all' | OrderStatus) => {
    setLoading(true);
    const params = status === 'all' ? {} : { status };
    api.get('/orders/kitchen', { params })
  ...
  const respond = async (id: string, action: 'accept' | 'reject') => {
    setProcessingId(id);
    try {
      await api.patch(`/orders/${id}/respond`, { action });
      toast.success(action === 'accept' ? 'অর্ডার গ্রহণ করা হয়েছে' : 'অর্ডার প্রত্যাখ্যান করা হয়েছে');
      loadOrders(filter);
    } catch (err: unknown) {
  ...
  return (
    <div>
      ...
      <p className="text-stone-500 mb-6">আসা অর্ডারগুলো দেখুন ও পরিচালনা করুন</p>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-stone-600">
                    {typeof order.user === 'string' ? '' : `${order.user.name} · ${order.user.phone}`}
                  </p>
